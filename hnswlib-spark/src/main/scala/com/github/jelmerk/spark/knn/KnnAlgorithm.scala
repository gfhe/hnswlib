...
private[knn] abstract class KnnAlgorithm[TModel <: KnnModelBase[TModel]](override val uid: String)
  ...
  private var initialModel = Option.empty[TModel]
  ...
  def setInitialModel(model: TModel): this.type = {
  ...
  private def typedFit[
    ...
    val initialModelOutputDir = initialModel.map(_.outputDir)
          ...
          val existingIndexOption = initialModelOutputDir
            .flatMap { dir =>
              ...
              if (fs.exists(indexPath)) Some {
                ...
                loadIndex[TId, TVector, TItem, TDistance](inputStream)
              } else {
